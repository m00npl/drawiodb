...
class GolemDB {
     ...
     */
    generateEntityKey() {
        // Generate a proper 32-byte (64 hex chars) entity key
    ...
    /**
     * Load diagram by ID (convenience method)
     */
    async loadDiagram(diagramId) {
        console.log('📥 Loading diagram via custom SDK:', diagramId);

        try {
            const entity = await this.getEntity(diagramId);

            if (!entity || !entity.parsedData) {
                throw new Error('Diagram not found or invalid data');
            }
