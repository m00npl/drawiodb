...
(function() {
    ...
    const LIBRARIES = {
        ...
        // Note: arkiv-sdk NOT needed - plugin uses backend API instead
    };
    ...
    async function loadAllLibraries() {
        try {
            ...
            // arkivSDK not needed - plugin uses backend API
    ...
    console.log('🔥🔥🔥 ARKIV PLUGIN SCRIPT LOADED!');
        ...
        Draw.loadPlugin(function(ui) {
            ...
            function createModal(title, message, buttons = [{ text: 'OK', style: 'primary' }]) {
                return new Promise((resolve) => {
                    ...
                    const existingModal = document.querySelector('.arkiv-modal-overlay');
                    if (existingModal) {
                    ...
                    if (!document.querySelector('#arkiv-modal-animations')) {
                        const style = document.createElement('style');
                        style.id = 'arkiv-modal-animations';
                        style.textContent = `
            ...
            // ===== GOLEM SDK FUNCTIONS =====
            async function initializeArkivSDK() {
                try {
            ...
            async function ensureArkivNetwork() {
                try {
            ...
            async function initArkivDB() {
                if (!arkivDB) {
                    // Ensure ethers is loaded first
                    await loadExternalLibrary('ethers');

                    arkivDB = new window.GolemDB({
                        rpcUrl: 'https://kaolin.hoodi.arkiv.network/rpc',
                        chainId: 0xE0087F821
                    });
                    await arkivDB.connect();
            ...
            async function saveToArkivViaSdk(xmlString, diagramId, title, author, encrypted = false) {
                try {
            ...
            async function loadFromArkiv(diagramId) {
                try {
                    ...
                    const result = await arkivDB.loadDiagram(diagramId);
            ...
            async function loadFromArkivSDK(diagramId) {
                try {
                    if (!arkivClient) {
            ...
            async function listFromArkivSDK() {
                try {
                    if (!arkivClient) {
                        throw new Error('Golem SDK not initialized');
                    }
            ...
            function createWalletStatusDisplay() {
                ...
                statusDiv.id = 'arkiv-wallet-status';
            ...
            window.arkivConnectWallet = connectWallet;
            window.arkivDisconnectWallet = disconnectWallet;
            ...
            async function ensureAuthentication() {
                ...
                const savedChoice = localStorage.getItem('arkiv-auth-choice');
                let choice = null;
                ...
                if (savedChoice) {
                    ...
                    localStorage.setItem('arkiv-auth-choice', choice);
            ...
            const saveToArkiv = withOperationLock(async function() {
                try {
                ...
                try {
                    ...
                    try {
                        ...
                        if (sdkAvailable) {
                            ...
                            loadResult = await loadFromArkiv(selectedDiagram.id);
                        }
                    } catch (sdkError) {
            ...
            const showArkivLoadDialog = showLoadDialog;
            ...
            const showShareDialog = withOperationLock(async function() {
                try {
                    ...
                    style.textContent = `
                        ...
                        .arkiv-modal-overlay label:hover {
            ...
            async function showConfigDialog() {
                    ...
                    <div style="margin-bottom: 15px; border-top: 1px solid #eee; padding-top: 15px;">
                        ...
                        <div style="margin-bottom: 8px;">
                            <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 5px;">
                                <input type="radio" name="authChoice" value="wallet" ${localStorage.getItem('arkiv-auth-choice') === 'wallet' ? 'checked' : ''} style="margin-right: 8px;">
                            ...
                            <label style="display: flex; align-items: center; cursor: pointer; margin-bottom: 5px;">
                                <input type="radio" name="authChoice" value="custodial" ${localStorage.getItem('arkiv-auth-choice') === 'custodial' ? 'checked' : ''} style="margin-right: 8px;">
                            ...
                            <label style="display: flex; align-items: center; cursor: pointer;">
                                <input type="radio" name="authChoice" value="" ${!localStorage.getItem('arkiv-auth-choice') ? 'checked' : ''} style="margin-right: 8px;">
                ...
                document.getElementById('saveConfig').onclick = async () => {
                    ...
                    if (selectedAuthChoice) {
                        localStorage.setItem('arkiv-auth-choice', selectedAuthChoice);
                        console.log('💾 Saved auth choice:', selectedAuthChoice);
                    } else {
                        localStorage.removeItem('arkiv-auth-choice');
            ...
            window.showArkivConfig = showConfigDialog;
            ...
            async function refreshDiagramList() {
                try {
                    const modal = document.querySelector('.arkiv-modal-overlay');
            ...
            function showManagerModal(diagrams) {
                // Create overlay
                const overlay = document.createElement('div');
                overlay.className = 'arkiv-modal-overlay';
                overlay.style.cssText = `
                    position: fixed;
                    top: 0;
                    left: 0;
            ...
            async function importDiagram(diagramId) {
                ...
                try {
                    if (connectionMode === 'sdk' && window.arkivSdk) {
                        console.log('🔄 Using SDK mode for loading');
                        loadResult = await loadFromArkiv(diagramId);
                    }
                } catch (sdkError) {
                    console.warn('⚠️ SDK load failed, falling back to backend:', sdkError);
                }
            ...
            // Add actions first
            ui.actions.addAction('arkiv-wallet', async function() {
                if (walletConnected) {
                    showWalletInfoModal();
                } else {
                    connectWallet();
            ...
            ui.actions.addAction('arkiv-save', function() {
                saveToArkiv();
            }, null, null, '💾 Save to Arkiv');

            ui.actions.addAction('arkiv-load', function() {
                showLoadDialog();
            }, null, null, '📂 Open from Arkiv');

            ui.actions.addAction('arkiv-manager', function() {
                openWebManager();
            }, null, null, '🌐 Arkiv Manager');

            ui.actions.addAction('arkiv-share', function() {
                showShareDialog();
            }, null, null, '🔗 Share Diagram');


            ui.actions.addAction('arkiv-config', function() {
                showConfigDialog();
            ...
            async function configureArkivNetwork() {
                try {
                    ...
                    const sdkReady = await initializeArkivSDK();
            ...
            if (fileMenu && fileMenu.funct) {
                ...
                fileMenu.funct = function(menu, parent) {
                    ...
                    menu.addItem = function(label, icon, action, parent, altText, elt, isEnabled) {
                        ...
                        if (!saveGroupFound && (
                            ...
                            originalAddItem.call(this, '💾 Save to Arkiv', null, function() {
                                ui.actions.get('arkiv-save').funct();
                            }, parent);
                            saveGroupFound = true;
                        }
                        ...
                        if (!openGroupFound && (
                            ...
                            originalAddItem.call(this, '📂 Open from Arkiv', null, function() {
                                ui.actions.get('arkiv-load').funct();
                            }, parent);
                            openGroupFound = true;
                        }
                    ...
                    menu.addItem('🔐 MetaMask Wallet', null, function() {
                        ui.actions.get('arkiv-wallet').funct();
                    }, parent);
            ...
            if (originalGetPopupMenu && originalGetPopupMenu.funct) {
                ...
                originalGetPopupMenu.funct = function(menu, cell, evt) {
                    ...
                    menu.addItem('🔗 Share Diagram', null, function() {
                        ui.actions.get('arkiv-share').funct();
                    });
            ...
            console.log('');
            console.log('🚀 ========================================');
            console.log('🌟    ARKIV PLUGIN FOR DRAW.IO       ');
            console.log('🚀 ========================================');
            ...
            setTimeout(() => {
                try {
                    // Create Arkiv menu
                    ui.menus.put('arkiv', new Menu(function(menu, parent) {
                    ...
                    if (menubar) {
                        ...
                        const arkivMenuItem = document.createElement('div');
                        arkivMenuItem.className = 'geMenuItem';
                        arkivMenuItem.style.cssText = `
                            ...
                            color: inherit;
                        `;
                        arkivMenuItem.textContent = 'Arkiv';

                        // Add hover effects
                        arkivMenuItem.onmouseenter = () => {
                            arkivMenuItem.style.background = 'rgba(0,0,0,0.1)';
                        };
                        arkivMenuItem.onmouseleave = () => {
                            arkivMenuItem.style.background = 'transparent';
                        };

                        // Add click handler to show menu
                        arkivMenuItem.onclick = (e) => {
                            e.preventDefault();
                            e.stopPropagation();

                            // Create dropdown menu
                            const arkivMenu = ui.menus.get('arkiv');
                            if (arkivMenu && arkivMenu.funct) {
                                ...
                                const rect = arkivMenuItem.getBoundingClientRect();
                                ...
                                const closeMenu = (e) => {
                                    if (!menuDiv.contains(e.target) && !arkivMenuItem.contains(e.target)) {
                                        safeRemoveElement(menuDiv);
                        ...
                        if (helpMenu) {
                            menubar.insertBefore(arkivMenuItem, helpMenu);
                        } else {
                            menubar.appendChild(arkivMenuItem);
                        }
                            ...
                            setTimeout(() => {
                                ...
                                if (container) {
                                    ...
                                    if (storageContainer) {
                                        ...
                                        const arkivBtn = document.createElement('div');
                                        arkivBtn.style.cssText = `
                                            display: inline-block;
                                            margin: 10px;
                                            padding: 20px;
                                        ...
                                        arkivBtn.onmouseenter = () => {
                                            arkivBtn.style.background = '#dcfce7';
                                            arkivBtn.style.borderColor = '#15803d';
                                        };

                                        arkivBtn.onmouseleave = () => {
                                            arkivBtn.style.background = '#f0fdf4';
                                            arkivBtn.style.borderColor = '#16a34a';
                                        };
                                        ...
                                        storageContainer.appendChild(arkivBtn);
                            ...
                            setTimeout(() => {
                                ...
                                for (let select of selects) {
                                    ...
                                    if (select.options.length > 0 && (
                                        ...
                                        // Add Arkiv option
                                        const arkivOption = document.createElement('option');
                                        arkivOption.value = 'arkiv';
                                        arkivOption.text = '🔗 Arkiv';
                                        select.appendChild(arkivOption);
    ...
    function exportGlobalFunctions() {
        window.saveToArkiv = async function(title, encrypted = false) {
        const authenticated = await ensureAuthentication();
        if (!authenticated) {
        ...
        try {
            ...
            // This is a simple wrapper - actual implementation is in the main saveToArkiv function
            // For now, just show an alert to use the Arkiv menu
            await showAlert('💡 Use Arkiv Menu', 'Please use File > Arkiv > Save to save your diagram.');
        } catch (error) {
            console.error('Save failed:', error);
        ...
        console.log('🌐 Global functions exported:', {
            saveToArkiv: typeof window.saveToArkiv,
            openWebManager: typeof window.openWebManager,
            showArkivConfig: typeof window.showArkivConfig
        });
    }
    ...
    const ensureExport = setInterval(() => {
        ...
        const allExported = typeof window.saveToArkiv === 'function' &&
                           typeof window.openWebManager === 'function' &&
                           typeof window.showArkivConfig === 'function';
